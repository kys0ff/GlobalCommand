...
private fun handleInstallCommand(positionals: List<String>, flags: List<String>) {
    ...
    when {
        ...
        pkgFile.isDirectory() ->
            installDirectory(pkgFile, positionals.getOrNull(2), deleteOriginal)
...
private fun installDirectory(dir: File, exeNameOverride: String?, deleteOriginal: Boolean) {
    val exeName = exeNameOverride ?: dir.name
    try {
        if (deleteOriginal) {
        ...
        // Linking executable
        println("${CYAN}●${RESET} Linking executable to ${BOLD}$BIN_PATH$exeName${RESET}")
        val linkFile = File("$BIN_PATH$exeName")
        if (linkFile.exists()) {
            printError("A file named '$exeName' already exists in $BIN_PATH.")
            println("Please remove it before reinstalling, or provide a custom executable name.")
            exitProcess(1)
        }

        val exeFile = File(LIB_PATH, exeName).listFiles()?.find { it.isFile() && it.isRunnable() && it.name == exeName }
            ?: run {
                printError("No executable named '$exeName' found in directory: $LIB_PATH$exeName")
                exitProcess(1)
            }
